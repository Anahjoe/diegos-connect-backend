...
router.post('/', authMiddleware, upload.single('photo'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const photoResult = await pool.query(
      `INSERT INTO photos (user_id, profile_id, url, display_order)
       VALUES ($1, $2, $3, $4)
       RETURNING id, url, display_order`,
      [userId, profileId, uploadResult.secure_url, displayOrder]
    );

    // If this is the first photo, set it as primary
    if (displayOrder === 0) {
      await pool.query(
        'UPDATE profiles SET primary_photo_url = $1 WHERE user_id = $2',
...
router.get('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, url, display_order FROM photos
       WHERE user_id = $1
...
// Delete a photo
router.delete('/:photoId', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.userId;
