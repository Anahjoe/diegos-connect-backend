  ...
  try {
    ...
    if (existing.rows.length > 0) {
      ...
      const result = await pool.query(
        `UPDATE profiles
         ...
         WHERE user_id = $10
         RETURNING *`,
        [first_name, bio, age, gender, city, latitude, longitude, looking_for, interests, userId]
      );
    ...
    const result = await pool.query(
      `INSERT INTO profiles
       ...
       RETURNING *`,
      [userId, first_name, bio, age, gender, city, latitude, longitude, looking_for, interests]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE profiles
       ...
       WHERE user_id = $4
       RETURNING *`,
      [bio, interests, looking_for, userId]
    );
