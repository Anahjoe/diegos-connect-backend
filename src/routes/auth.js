...
router.post('/send-otp', async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO verification_codes (phone_number, code, expires_at)
       VALUES ($1, $2, $3)`, [phone_number, code, expiresAt]);
...
router.post('/verify-otp', async (req, res) => {
    try {
        ...
        const otpResult = await pool.query(`SELECT id FROM verification_codes
       WHERE phone_number = $1 AND code = $2 AND expires_at > NOW() AND is_used = false`, [phone_number, code]);
        if (otpResult.rows.length === 0) {
            return res.status(400).json({ error: 'Invalid or expired OTP' });
        }
        await pool.query(`UPDATE verification_codes SET is_used = true, used_at = NOW()
       WHERE id = $1`, [otpResult.rows[0].id]);
        ...
        if (userResult.rows.length === 0) {
            ...
            const newUserResult = await pool.query(`INSERT INTO users (phone_number, username)
         VALUES ($1, $2)
