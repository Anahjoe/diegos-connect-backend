...
router.get('/stats', adminMiddleware, async (req, res) => {
    try {
        const totalUsers = await pool.query("SELECT COUNT(*) FROM users WHERE status = 'active'");
        ...
        const cities = await pool.query(`SELECT city, COUNT(*) as count FROM profiles
...
router.get('/reports', adminMiddleware, async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT r.id, r.report_type, r.description, r.status, r.created_at,
...
router.post('/reports/:reportId/resolve', adminMiddleware, async (req, res) => {
    try {
        ...
        const reportResult = await pool.query('SELECT reported_user_id FROM reports WHERE id = $1', [reportId]);
        ...
        if (action === 'suspended') {
            await pool.query("UPDATE users SET status = 'suspended' WHERE id = $1", [reportedUserId]);
        }
        else if (action === 'deleted') {
            await pool.query("UPDATE users SET status = 'deleted' WHERE id = $1", [reportedUserId]);
        }
