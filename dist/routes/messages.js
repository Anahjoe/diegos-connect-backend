...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT m.id AS match_id, m.matched_at,
...
router.get('/:matchId/messages', authMiddleware, async (req, res) => {
    try {
        ...
        const matchCheck = await pool.query(`SELECT id FROM matches
       WHERE id = $1 AND (user_id_1 = $2 OR user_id_2 = $2)`, [matchId, userId]);
        if (matchCheck.rows.length === 0) {
            return res.status(403).json({ error: 'Not part of this match' });
        }
        const result = await pool.query(`SELECT id, sender_id, recipient_id, text, is_read, created_at
       FROM messages
       WHERE match_id = $1
...
router.post('/:matchId/messages', authMiddleware, async (req, res) => {
    try {
        ...
        const matchCheck = await pool.query(`SELECT user_id_1, user_id_2 FROM matches
       WHERE id = $1 AND (user_id_1 = $2 OR user_id_2 = $2) AND status = 'active'`, [matchId, userId]);
        if (matchCheck.rows.length === 0) {
            return res.status(403).json({ error: 'Not part of this match' });
        }
        const match = matchCheck.rows[0];
        const recipientId = match.user_id_1 === userId ? match.user_id_2 : match.user_id_1;
        const result = await pool.query(`INSERT INTO messages (match_id, sender_id, recipient_id, text)
       VALUES ($1, $2, $3, $4)
       RETURNING id, text, created_at, is_read`, [matchId, userId, recipientId, text]);
...
router.post('/:matchId/read', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { matchId } = req.params;
        const result = await pool.query(`UPDATE messages
       SET is_read = true, read_at = NOW()
       WHERE match_id = $1 AND recipient_id = $2 AND is_read = false`, [matchId, userId]);
