...
router.post('/like', authMiddleware, async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO swipes (swiper_id, swiped_id, action)
       VALUES ($1, $2, 'like')
       ON CONFLICT (swiper_id, swiped_id) DO NOTHING`, [userId, swiped_id]);
        // Check if the other person already liked us
        const mutualCheck = await pool.query(`SELECT id FROM swipes
       WHERE swiper_id = $1 AND swiped_id = $2 AND action = 'like'`, [swiped_id, userId]);
        if (mutualCheck.rows.length > 0) {
            // It's a match! Create it
            const matchResult = await pool.query(`INSERT INTO matches (user_id_1, user_id_2)
         VALUES ($1, $2)
         RETURNING id, matched_at`, [userId, swiped_id]);
            // Get the matched user's basic info
            const matchedUser = await pool.query(`SELECT first_name, primary_photo_url FROM profiles WHERE user_id = $1`, [swiped_id]);
            return res.json({
                success: true,
                action: 'like',
                is_match: true,
...
// Pass on a user
router.post('/pass', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { swiped_id } = req.body;
        if (!swiped_id) {
