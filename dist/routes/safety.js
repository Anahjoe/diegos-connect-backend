...
router.post('/block', authMiddleware, async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO blocks (blocker_id, blocked_id, reason)
       VALUES ($1, $2, $3)
       ON CONFLICT (blocker_id, blocked_id) DO NOTHING`, [userId, blocked_id, reason || null]);
        // End any active match between them
        await pool.query(`UPDATE matches SET status = 'blocked'
       WHERE (user_id_1 = $1 AND user_id_2 = $2)
          OR (user_id_1 = $2 AND user_id_2 = $1)`, [userId, blocked_id]);
...
router.get('/blocks', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const result = await pool.query(`SELECT b.blocked_id, b.reason, b.created_at, p.first_name
       FROM blocks b
...
router.post('/report', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO reports (reporter_id, reported_user_id, report_type, description)
       VALUES ($1, $2, $3, $4)
       RETURNING id`, [userId, reported_user_id, report_type, description || null]);
        res.json({
            success: true,
