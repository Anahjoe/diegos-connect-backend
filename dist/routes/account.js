...
router.get('/settings', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT id, username, phone_number, email, status, created_at
...
router.post('/deactivate', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        await pool.query("UPDATE users SET status = 'deactivated', updated_at = NOW() WHERE id = $1", [userId]);
